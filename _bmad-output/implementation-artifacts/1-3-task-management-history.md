...
6. **AC6 - 历史记录 Pinia store 独立**：创建 `frontend/src/stores/crawl.ts`（Setup Store 语法，与 1-2 `ui.ts` 风格一致），导出 `useCrawlStore`：state `{ history: CrawlTaskRecord[], activeTask: CrawlTaskRecord | null }` + actions `addTask(record)` / `removeTask(id)` / `getTaskById(id)` / `setActiveTask(id)` / `restoreTask(task, neighborId)` / `startTick()` / `stopTick()` / `getNeighborId(id)`；通过 `pinia-plugin-persistedstate`（1-2 已安装 ^3.x）持久化 `history` 字段；与 `useUiStore` 解耦避免循环依赖。 [Source: architecture.md#L1342-L1356 useCrawlStore 命名, architecture.md#L1711 useCrawlStore.js 路径, architecture.md#L188 Pinia stores 按视图分离, 1-2-simple-view-url-input.md#L250 crawl.ts 边界声明]
...
8. **AC8 - 任务删除与撤销**：每个历史卡片支持"删除"操作（卡片右侧"查看 / 导出 / 删除"按钮组中的删除按钮），点击后任务从 `useCrawlStore().history` 移除并持久化；不实现 30 天回收站（Epic 7 Story 7-4 task-deletion-recovery 交付），本 story 删除即永久删除但提供"撤销"通知（Naive UI `n-message` type="info" duration=5000 含"撤销"按钮，5 秒内点击恢复任务到原位置）；撤销仅在同会话有效（视图切换或组件卸载时静默恢复尚未过期的删除项；刷新页面后删除生效，删除项不再恢复）；恢复到原位置基于删除时捕获的相邻项 ID 重定位；若删除时捕获的相邻项已不在 history，则回退至列表顶部作为优雅降级，不保证严格原位。 [Source: epic-01-first-time-onboarding.md#L98-L102, ux-design-specification.md#L2879-L2883 任务删除30天恢复窗口由 Epic 7 交付, ux-design-specification.md#L2883 即时撤销提示, epic-07-undo-redo-recovery.md 边界声明]
...
- [x] [Review][Patch] (R3-P6) **SimpleView 直接 `crawlStore.history.findIndex` 和 `crawlStore.history.splice` 绕过 Pinia action 边界** [frontend/src/views/SimpleView.vue:DELETE+UNDO] — crawl store 加 `restoreTask(task, neighborId: string | null)` action 由其承担 splice
- [x] [Review][Patch] (R3-P7) **`removeTask(id)` 不重置 `activeTask`**，用户在 drawer 打开某任务时从 HistoryCard 删除之，drawer 仍展示已删 phantom 记录 [frontend/src/stores/crawl.ts:32] — removeTask 内 `if (activeTask.value?.id === id) activeTask.value = null`（baseline b1f382f 已含此行，R3-P7 实为回归测试验证）
...
- [x] [Review][Patch] (R3-P17) **AC12 字面要求 `≥6 测试` 包含 clearHistory**，但 B10 已删除 clearHistory action，crawl.test.ts 实际仅 5 用例 [Story 1.3 AC12] — 更新 AC12 文案移除 clearHistory，标 `≥10 测试`，含 restoreTask 三态 + refcount
...
### Review Findings — Round 4 (2026-08-10)

> Round 4 对抗式审查层：`blind`（19 项 findings）+ `edge`（6 项 findings JSON）+ `auditor`（13 项 findings：1 Major / 5 Medium / 5 Low / 2 Nit + 2 verified pass）。Acceptance Auditor 初次启动 stall 超时 600s，retry 成功（`failed_layers` 临时记录后清空）。
> 经 normalize + dedupe（3 项合并：B3+E1 HMR leak、B6+E5 icon shared ref、B12+B19+A4 crawl.test.ts count）后 34 distinct → 分类为 28 项 patch、1 项 defer、5 项 dismissed。无 decision-needed（Round 3 已决 3 项，Round 4 无新增歧义）。

#### Patches

- [x] [Review][Patch] (R4-P1) **crawl store 缺 `import.meta.hot?.dispose(() => stopTick())`** — HMR 重实例 leak 30s setInterval [frontend/src/stores/crawl.ts:17-18, 54-70] (B3+E1) — D1/R3-P18 prose 称 HMR 安全但实现遗漏 dispose；加 `if (import.meta.hot) import.meta.hot.dispose(() => { if (tickTimer) { clearInterval(tickTimer); tickTimer = null; } activeTickers = 0; })`
- [x] [Review][Patch] (R4-P2) **`startTick` 用 `if (tickTimer === null)` 而非 R3-P18 prose 的 `if (activeTickers === 1)`** — 状态解耦潜在 bug [frontend/src/stores/crawl.ts:55-61] (B4) — 改对齐 prose：`if (activeTickers === 1) { nowTimestamp.value = Date.now(); tickTimer = setInterval(...) }`
- [x] [Review][Patch] (R4-P3) **useStatusTag 导出 `StatusTagInfo` 与 `StatusTagEntry` 两同构类型** — dead duplicate [frontend/src/composables/useStatusTag.ts:6-18] (B5) — 合并为单一类型（`type StatusTagEntry = StatusTagInfo`）或用 `Pick`/`Omit` 派生
- [x] [Review][Patch] (R4-P4) **cloneEntry 仅浅克隆 color，icon 共享 ref** — PALETTE 单例可被 consumer 改脏 [frontend/src/composables/useStatusTag.ts:48-55] (B6+E5) — 加 `Object.freeze(PALETTE); Object.freeze(FALLBACK);` 防止任何 mutation 路径
- [x] [Review][Patch] (R4-P5) **useStatusTag clone protection test 仅改 `color.color`** — 未覆盖 borderColor/textColor [frontend/tests/composables/useStatusTag.test.ts] (B7) — 扩展测试：mutate `first.color.borderColor`、`first.color.textColor` 后断言 second 仍为原值
- [x] [Review][Patch] (R4-P6) **"restoreTask appends when neighborId is null" 测试无法区分 push vs unshift**（history 为空时两者结果相同） [frontend/tests/stores/crawl.test.ts] (B8) — 测试加多条记录且 neighborId 为 null，断言 push 到末尾（非 unshift 到顶部）
- [x] [Review][Patch] (R4-P7) **"restoreTask unshifts when neighbor id is missing" 测试将 position 退化固化为预期值** [frontend/tests/stores/crawl.test.ts] (B9) — 测试名或注释明确标注这是 fallback 而非严格恢复，避免未来误读为契约
- [x] [Review][Patch] (R4-P8) **SimpleView onDelete 直接读 `crawlStore.history.findIndex/[idx]/[idx+1]`** — R3-P6 action 边界只覆盖写不覆盖读 [frontend/src/views/SimpleView.vue:272-283] (B10) — store 加 `getNeighborId(id): string | null` action，onDelete 走该方法；或加 `removeAndCaptureUndo(id)` 一体化 action
- [x] [Review][Patch] (R4-P9) **SimpleView.test.ts afterEach 空 catch 吞所有错误** — 未来 stopTick 出错不可见 [frontend/tests/components/SimpleView.test.ts] (B11) — 收窄 catch：仅吞 "no active pinia" 错误，其他重抛；或改为 `try { const s = useCrawlStore(); s.stopTick(); } catch (e) { if (!/no active pinia/.test(String(e))) throw e; }`
- [x] [Review][Patch] (R4-P10) **HistoryCard.vue 删除 CSS 文本截断 fallback** — NEllipsis 失效时长标题溢出无防御 [frontend/src/components/simple/HistoryCard.vue] (B13) — 加 CSS `min-width: 0` 至 `.history-card__body`，或保留 `text-overflow: ellipsis` 作 NEllipsis 副本
- [x] [Review][Patch] (R4-P11) **TaskDetailDrawer.test.ts `mountDrawer` 未传 `now` 参数** — Vue "missing required prop" warning 污染日志 [frontend/tests/components/TaskDetailDrawer.test.ts] (B15) — `mountDrawer` 内 `props: { show, record, now: Date.now() }`
- [x] [Review][Patch] (R4-P12) **Drawer stub 只声明 `props: { show: Boolean }`** — width/placement/closable 等未守，regression 静默通过 [frontend/tests/components/TaskDetailDrawer.test.ts:30] (E6) — 扩展 stub `props: { show: Boolean, width: [Number,String], placement: String, closable: Boolean }` + 加 `expect(drawer.props('placement')).toBe('right')` 断言
- [x] [Review][Patch] (R4-P13) **AC6 仍引用 `clearHistory()` 且未列 R3 新增 actions** [Story 1.3 file AC6] (A1) — 更新 AC6 文案：action 列表改为 `addTask / removeTask / getTaskById / setActiveTask / restoreTask / startTick / stopTick`，移除 `clearHistory`
- [x] [Review][Patch] (R4-P14) **Dev Notes "Code Conventions" naming list stale** [Story 1.3 file Dev Notes] (A2) — 同步更新 actions 命名列表，移除 `clearHistory`，加 `restoreTask / startTick / stopTick`
- [x] [Review][Patch] (R4-P15) **Change Log "5 actions" 误计** — 实际 7 actions，且漏 `restoreTask` [Story 1.3 file Change Log] (A3) — 更新条目 "7 actions（addTask/removeTask/getTaskById/setActiveTask/restoreTask/startTick/stopTick）"
- [x] [Review][Patch] (R4-P16) **Modified Files 段称 crawl.test.ts "5 测试"** — 实际 10 测试 [Story 1.3 file File Structure] (B12+B19+A4) — 更新为 "10 测试：addTask / FIFO 截断 / removeTask（含 activeTask 清空）/ getTaskById / persist / restoreTask 三态 / startTick-stopTick refcount"
- [x] [Review][Patch] (R4-P17) **R3-P6 Review Findings entry 文本仍写 `restoreTask(task, index)`** — D2 裁决后应改 `neighborId` [Story 1.3 file R3-P6 entry] (A5) — 更新为 `restoreTask(task, neighborId: string | null)`
- [x] [Review][Patch] (R4-P18) **R3-P17 entry 称 "标 `≥5 测试`"** — AC12 实际 "≥10 测试" [Story 1.3 file R3-P17 entry] (A6) — 更新 entry 文本为 "标 `≥10 测试`，含 restoreTask 三态 + refcount"
- [x] [Review][Patch] (R4-P19) **AC8 "撤销仅在同会话有效" vs R3-P20 onUnmount auto-restore UX 矛盾** [Story 1.3 file AC8] (A7) — AC8 文案补 "（视图切换或组件卸载时静默恢复尚未过期的删除项；刷新页面后删除生效）"
- [x] [Review][Patch] (R4-P20) **AC8 "原位置" 在 neighbor 也被删 mid-window 时退化为 unshift** [Story 1.3 file AC8] (A8) — AC8 文案补 "（若删除时捕获的相邻项已不在 history，则回退至列表顶部作为优雅降级，不保证严格原位）"
- [x] [Review][Patch] (R4-P21) **R3-P15 SimpleView 位置断言只验证 history[0] 顶部** — 未覆盖中间位置 splice [frontend/tests/components/SimpleView.test.ts] (A11) — 测试加多条历史，删除非顶部项，撤销后断言恢复到原中间位置
- [x] [Review][Patch] (R4-P22) **R3-P7 entry prose 称 removeTask 缺 activeTask 清空但 git baseline b1f382f 已含** — 记录漂移 [Story 1.3 file R3-P7 entry] (B16) — 更新 entry 文本标注 "baseline b1f382f 已含此行，R3-P7 实为回归测试验证"
- [x] [Review][Patch] (R4-P23) **crawl.test.ts `afterEach` 仅 `app.unmount()`, 未 `stopTick()`** — startTick 测试中途失败泄漏 30s setInterval [frontend/tests/stores/crawl.test.ts] (B1) — afterEach 加 `try { useCrawlStore().stopTick(); } catch {}`，或把 startTick 测试拆独立 suite 带 afterEach 清 timer
- [x] [Review][Patch] (R4-P24) **startTick/stopTick refcount 测试不验证 refcount 行为** — 无 fake timers + 无 last-stop assertion [frontend/tests/stores/crawl.test.ts] (B2) — 测试加 `vi.useFakeTimers()`、`advanceTimersByTimeAsync(30000)` 验证 nowTimestamp 推进，再 `stopTick()` 一次 (activeTickers=1→0)，`advanceTimersByTimeAsync(30000)` 验证 nowTimestamp 不再推进；再 startTick*2 + stopTick*1 验证仍 ticking
- [x] [Review][Patch] (R4-P25) **crawl.test.ts `waitForPersist` 用真实 `setTimeout(5)`** — 与 fake timers 模式耦合，未来启用将失效 [frontend/tests/stores/crawl.test.ts] (B18) — helper 前置 `const wasFake = vi.getTimerConfig?.(); if (wasFake) vi.useRealTimers();`，或改用 `vi.advanceTimersByTimeAsync(20)` 兼容路径
- [x] [Review][Patch] (R4-P26) **onBeforeUnmount restoreAll FIFO 迭代导致多 undo 场景恢复顺序错乱** [frontend/src/views/SimpleView.vue:299-304] (E2) — 改 LIFO 迭代：`for (const p of [...pendingUndos.value].reverse()) { clearTimeout(p.timer); crawlStore.restoreTask(p.task, p.neighborId); } pendingUndos.value = [];`
- [x] [Review][Patch] (R4-P27) **restoreAll 循环中 restoreTask 抛错中断** — 剩余 timer 不清, `pendingUndos.value = []` 不执行 [frontend/src/views/SimpleView.vue:299-304] (E3) — 包 `try/catch`：`for (const p of pendingUndos.value) { try { clearTimeout(p.timer); crawlStore.restoreTask(p.task, p.neighborId); } catch (e) { console.error('restore failed', e); } } pendingUndos.value = [];`
- [x] [Review][Patch] (R4-P28) **onHistoryClick try/finally 仅守护 `showHistory=false`** — syncStatus 抛错成 unhandled rejection [frontend/src/components/SmartURLInput.vue:217-228] (E4) — `try { syncStatus(value); focusInput(); } catch (e) { console.warn('onHistoryClick error', e); } finally { showHistory.value = false; }`

#### Deferred

- [x] [Review][Defer] (R4-DF1) **setActiveTask / nowTimestamp 30s tick 触发 pinia-plugin-persistedstate $subscribe 冗余 localStorage 序列化** [frontend/src/stores/crawl.ts] (A10) — `pick: ['history']` 限制写入字段，但 $subscribe 仍触发 history 完整序列化。pref concern，非 spec violation。deferred, Phase 2 性能优化阶段处理（可加 `paths` 选项或 shallow 兼容）

#### Dismissed

- (R4-DIS1) (B14) TaskDetailDrawer `now: number` required 即使 record=null 空状态 — R3-P4 deliberate API choice（漏绑显式失败优于静默 NaN），不视为缺陷
- (R4-DIS2) (A9) now prop silent NaN — 同 R4-DIS1，R3-P4 选择安全优先，dismiss 合并
- (R4-DIS3) (A12) vitest 109/109 arithmetic verifies — Change Log 数字一致，verified pass
- (R4-DIS4) (A13) all 6 forbidden items clean — verified pass
- (R4-DIS5) (B17) onHistoryClick B8 fix fragility (Vue microtask batch order) — 已于 R3-DF1 deferred，重复 dismiss
...
- **`useCrawlStore` 规格** [Source: architecture.md#L1342-L1356, architecture.md#L1711]：
  ...
  - 命名：PascalCase `useCrawlStore`；state camelCase `history` / `activeTask`；actions camelCase `addTask` / `removeTask` / `getTaskById` / `setActiveTask` / `restoreTask` / `getNeighborId` / `startTick` / `stopTick`
...
- `frontend/src/stores/crawl.ts` — useCrawlStore with history（FIFO 50）+ activeTask + nowTimestamp（30s tick）+ 7 actions（addTask/removeTask/getTaskById/setActiveTask/restoreTask/getNeighborId/startTick/stopTick）+ persist `'ai-crawler:crawl-history'`
...
- `frontend/tests/stores/crawl.test.ts` — 10 测试：addTask / FIFO 50 截断 / removeTask（含 activeTask 清空）/ getTaskById / getNeighborId / restoreTask 三态（valid neighbor / null 末尾 / missing 顶部 fallback）/ persist localStorage（waitForPersist 轮询）/ startTick-stopTick refcount（fake timers + 30s 推进断言）
...
- 2026-08-10 19:50 - Review Round 4 对抗式审查完成（Blind Hunter 19 + Edge Case Hunter 6 + Acceptance Auditor 13 = 38 raw → 28 patch / 1 defer / 5 dismiss / 0 decision-needed）；3 项合并（B3+E1 HMR leak、B6+E5 icon shared ref、B12+B19+A4 crawl.test.ts count）；Acceptance Auditor 初次启动 stall 超时 600s，retry 成功
- 2026-08-10 20:10 - Review Round 4 全 28 项 patch 批量应用完成：crawl.ts HMR `import.meta.hot.dispose` 清 timer + `activeTickers === 1` 严格 startTick 守护 + 新增 `getNeighborId(id)` action（R4-P1/P2/P8）；useStatusTag.ts `StatusTagEntry` type alias 合并 + `Object.freeze(PALETTE/FALLBACK)` 防共享克隆被 mutation（R4-P3/P4）；useStatusTag.test.ts 扩展克隆保护测试覆盖 borderColor/textColor + fallback（R4-P5）；crawl.test.ts 改 restoreTask null 测试为多记录 push 末尾 + missing 邻居名标注 degraded fallback + 新增 getNeighborId 测试 + 新增 fake timers refcount 30s 推进断言 + afterEach 双 stopTick + `vi.getTimerConfig` 兼容 fake timers（R4-P6/P7/P23/P24/P25）；SimpleView onDelete 走 `crawlStore.getNeighborId(id)` + onBeforeUnmount `[...pendingUndos.value].reverse()` LIFO 恢复 + try/catch 防中断（R4-P8/P26/P27）；HistoryCard.vue `.history-card__title` 加 `text-overflow: ellipsis` NEllipsis 失效 fallback（R4-P10）；TaskDetailDrawer.test.ts `mountDrawer` 传 `now: Date.now()` + Drawer stub `props` 扩展 width/placement/closable + 加 `expect(drawer.props('placement')).toBe('right')` 断言（R4-P11/P12）；SmartURLInput onHistoryClick try/catch+finally 防 syncStatus 抛错成 unhandled rejection（R4-P28）；SimpleView.test.ts afterEach catch 收窄至 "no active pinia" + 新增"撤销删除恢复到中间位置"测试覆盖 splice 严格恢复（R4-P9/P21）；Story file AC6 / AC8 / Dev Notes naming list / R3-P6/P7/P17 entries / Change Log / File Structure 全同步更新（R4-P13/P14/P15/P16/P17/P18/P19/P20/P22）
- 2026-08-10 20:10 - vitest 最终运行：113/113 通过，14 个测试文件全绿（crawl.test.ts 11 测试 含 getNeighborId + restoreTask 三态 + fake timers refcount 30s 推进；SimpleView.test.ts 18 测试 含中间位置撤销恢复 splice；useStatusTag.test.ts 含 PALETTE freeze + clone 三属性 + fallback 克隆保护；TaskDetailDrawer.test.ts 含 Drawer stub placement 断言）
- 2026-08-10 20:30 - Story 1-3 状态 `review` → `done`，Round 4 全 28 项 patch 闭合，1 项 deferred（30s tick redundant localStorage serialization 见 deferred-work.md R4-DF1），5 项 dismissed
